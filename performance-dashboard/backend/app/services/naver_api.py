...
class NaverAdsClient:
    ...
    def _put(self, uri: str, data: Any = None, customer_id: str | None = None) -> Any:
        """PUT 요청."""
        headers = self._headers("PUT", uri, customer_id)
        resp = requests.put(BASE_URL + uri, json=data, headers=headers, timeout=30)
        resp.raise_for_status()
        return resp.json()

    def _delete(self, uri: str, customer_id: str | None = None) -> Any:
        """DELETE 요청."""
        headers = self._headers("DELETE", uri, customer_id)
        resp = requests.delete(BASE_URL + uri, headers=headers, timeout=30)
        resp.raise_for_status()
        if resp.content:
            return resp.json()
        return {"success": True}
    ...
    def update_keyword(
        ...
        Parameters:
            ...
            bid_amount: 새 입찰가 (원, 10원 단위)
            status: "ELIGIBLE" (활성) | "PAUSED" (일시중지)
        """
        data: Dict[str, Any] = {"nccKeywordId": keyword_id}
        if bid_amount is not None:
            data["bidAmt"] = bid_amount
        if status is not None:
            data["userLock"] = status == "PAUSED"
        return self._put(
    ...
    def get_negative_keywords(
        self, customer_id: str, adgroup_id: str
    ) -> List[Dict[str, Any]]:
        """광고그룹의 제외 키워드 목록."""
        return self._get(
            "/ncc/negative-keywords",
            params={"nccAdgroupId": adgroup_id},
            customer_id=customer_id,
        )
    ...
    def add_negative_keywords(
        ...
        """제외 키워드 추가."""
        data = [
            {"nccAdgroupId": adgroup_id, "keyword": kw, "type": "NEGATIVE_EXACT"}
            for kw in keywords
        ]
        return self._post(
            "/ncc/negative-keywords",
    ...
    def delete_negative_keyword(
        self, customer_id: str, negative_keyword_id: str
    ) -> Dict[str, Any]:
        """제외 키워드 삭제."""
        return self._delete(
            f"/ncc/negative-keywords/{negative_keyword_id}",
            customer_id=customer_id,
        )
