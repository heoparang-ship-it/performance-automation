...
export default function AdsPage() {
  ...
  const toggleCampaign = useCallback(
    async (campaignKey: string) => {
      const target = treeRef.current.find((n) => n.key === campaignKey);
  ...
  const toggleAdgroup = useCallback(
    async (campaignKey: string, adgroupKey: string) => {
      const campaignNode = treeRef.current.find((n) => n.key === campaignKey);
...
const AdgroupRows = memo(function AdgroupRows({
  adgroup,
  campaignKey,
  cols,
  sortNodes,
