...
import { BarChart3, AlertTriangle, TrendingDown, Rocket, AlertCircle, Lightbulb, DollarSign, CheckCircle, Clock } from "lucide-react";
import {
  KpiSummary, ActionItem,
  CampaignPerfWithDelta,
  useDashboardSummary, useDashboardTrend, useDashboardAlerts,
  useDailySummaries, useCampaignsWithDelta, useAdgroups,
} from "@/lib/api";
...
export default function DashboardPage() {
  ...
  const { data: adgroups } = useAdgroups(selectedStoreId);
  const { data: dailyData } = useDailySummaries(selectedStoreId);
  ...
  const rawTrend = trendList.length > 0
    ? trendList
    : dailyList.length > 0
      ? [...dailyList].reverse().map((d) => ({
          date: d.date,
          cost: d.cost,
          revenue: d.revenue,
          clicks: d.clicks,
    ...
    <div className="p-4 max-w-[1400px] h-[calc(100vh)] flex flex-col overflow-hidden">
      ...
      <div className="grid grid-cols-5 gap-3 flex-1 min-h-0">
        ...
        <div className="col-span-3 flex flex-col gap-3 min-h-0">
          {/* 추이 차트 */}
          <div className="bg-gray-900 rounded-lg border border-gray-800 p-3 shadow-sm">
          ...
          {/* 캠페인 성과 테이블 */}
          <div className="bg-gray-900 rounded-lg border border-gray-800 p-3 shadow-sm flex-1 min-h-0 flex flex-col">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xs font-bold text-gray-200">
                캠페인 성과
                <span className="ml-1.5 text-[10px] font-normal text-gray-500">{periodDays}일 기준 · 이전 {periodDays}일 대비</span>
              </h3>
              <button onClick={() => setShowCampaignModal(true)} className="text-[10px] text-emerald-400 hover:text-emerald-300 font-medium">전체보기</button>
            </div>
              ...
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {sortedCampaigns.length === 0 ? (
                    <tr><td colSpan={7} className="text-center py-4 text-gray-500">캠페인 데이터가 없습니다</td></tr>
                  ) : sortedCampaigns.slice(0, 7).map((c, i) => (
                    <tr key={i} className="border-b border-gray-800 hover:bg-gray-800">
                      <td className="py-1.5 px-2 font-medium text-gray-100 truncate max-w-[180px]">{c.campaign_name || "(미분류)"}</td>
                      <td className="py-1.5 px-2 text-right text-gray-300">{formatKRWShort(c.cost)}</td>
                      <td className="py-1.5 px-2 text-right text-gray-300">{c.clicks}</td>
                      <td className="py-1.5 px-2 text-right text-gray-300">{c.ctr.toFixed(1)}%</td>
                      <td className={`py-1.5 px-2 text-right font-bold ${c.roas >= 200 ? "text-emerald-600" : c.roas > 0 ? "text-red-500" : "text-gray-500"}`}>{c.roas > 0 ? c.roas.toFixed(0) + "%" : "-"}</td>
        ...
        <div className="col-span-2 flex flex-col gap-3 min-h-0 overflow-y-auto">
          {/* 계정 건강도 */}
          <div className="bg-gray-900 rounded-lg border border-gray-800 p-3 shadow-sm flex-shrink-0">
            <h3 className="text-xs font-bold text-gray-200 mb-2 flex items-center gap-1"><BarChart3 className="w-5 h-5" /> 계정 건강도</h3>
          ...
          {/* 7일 일별 추이 + 전일대비 변화율 */}
          <DailyTrendBlock dailyData={dailyList} />
            ...
            <div className="bg-gray-900 rounded-lg border border-gray-800 p-3 shadow-sm flex-shrink-0">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-xs font-bold text-gray-200 flex items-center gap-1">
                  <AlertTriangle className="w-4 h-4" /> 긴급 알림
                  <span className="ml-1.5 px-1.5 py-0.5 bg-red-100 text-red-700 text-[10px] font-bold rounded-full">{highAlerts.length}</span>
                </h3>
                <button onClick={() => setShowAlertModal(true)} className="text-[10px] text-emerald-400 hover:text-emerald-300 font-medium">전체 {alertsList.length}건</button>
              </div>
            ...
            <tbody>
              {sortedAdgroups.map((ag, i) => (
                <tr key={i} className="border-b border-gray-800 hover:bg-gray-800">
                  ...
                  <td className="py-2.5 px-3 text-xs text-gray-400">{ag.campaign_name}</td>
                  ...
                  <td className={`py-2.5 px-3 text-right font-bold ${ag.roas >= 200 ? "text-emerald-600" : ag.roas > 0 ? "text-red-500" : "text-gray-500"}`}>{ag.roas > 0 ? formatPct(ag.roas) : "-"}</td>
...
function CompactKpi({ title, value, delta, inverseDelta }: { title: string; value: string; delta?: number | null; inverseDelta?: boolean }) {
  ...
  return (
    <div className="bg-gray-900 rounded-lg border border-gray-800 px-3 py-2.5 shadow-sm hover:shadow-md transition-shadow">
...
function DailyTrendBlock({ dailyData }: { dailyData: KpiSummary[] }) {
  ...
  return (
    <div className="bg-gray-900 rounded-lg border border-gray-800 p-3 shadow-sm flex-shrink-0">
      <h3 className="text-xs font-bold text-gray-200 mb-2">7일 일별 추이 <span className="font-normal text-gray-500">전일대비 변화율</span></h3>
      <div className="space-y-0.5">
        {days.map((day, i) => {
          ...
          return (
            <div key={day.date} className={`flex items-center gap-2 py-1 px-2 rounded text-[10px] ${isToday ? "bg-emerald-950/20" : i % 2 === 0 ? "bg-gray-800/50" : ""}`}>
              {/* 날짜 */}
              <div className="w-[52px] flex-shrink-0">
                <span className="font-bold text-gray-200">{day.date.slice(5)}</span>
                <span className="text-gray-500 ml-0.5">({dayName})</span>
                {isToday && <span className="text-emerald-400 text-[8px] ml-0.5"><Clock className="w-3 h-3 inline" /></span>}
              </div>

              {/* 핵심 지표 */}
              <div className="w-[45px] text-right flex-shrink-0">
                <span className="text-gray-400">{formatKRWShort(day.cost)}</span>
              </div>
              <div className="w-[55px] text-right flex-shrink-0">
                <span className={`font-bold ${day.revenue > 0 ? "text-gray-100" : "text-gray-600"}`}>{day.revenue > 0 ? formatKRWShort(day.revenue) : "-"}</span>
              </div>
              <div className="w-[38px] text-right flex-shrink-0">
                <span className={`font-bold ${day.roas >= 200 ? "text-emerald-600" : day.roas > 0 ? "text-amber-600" : "text-gray-600"}`}>{day.roas > 0 ? day.roas.toFixed(0) + "%" : "-"}</span>
              </div>

              {/* 변화율 바 그래프 */}
              <div className="flex-1 flex items-center gap-1 min-w-0">
                <div className="flex-1 h-[14px] relative bg-gray-800 rounded overflow-hidden">
                  {/* 중앙선 */}
                ...
                <div className="w-[42px] text-right flex-shrink-0">
                  {mainDelta != null ? (
                    <span className={`font-bold ${isPositive ? "text-emerald-600" : isNegative ? "text-red-500" : "text-gray-500"}`}>
  ...
  return (
    <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-lg p-3 shadow-sm flex-shrink-0">
      <div className="flex items-center gap-2 mb-2.5">
        <div className="w-5 h-5 bg-emerald-500 rounded flex items-center justify-center text-white font-black text-[9px]">X</div>
        <div>
          <h3 className="text-xs font-bold text-white">AI 진단</h3>
        ...
        {diagnoses.map((d, i) => (
          <div key={i} className={`rounded-lg p-2 ${d.bgColor}`}>
            <div className="flex items-start gap-1.5">
              <span className="text-xs mt-0.5 flex-shrink-0">{d.icon}</span>
              <div className="min-w-0 flex-1">
                ...
                {d.solution && (
                  <p className="text-[10px] text-emerald-300 font-medium mt-1">&rarr; {d.solution}</p>
                )}
...
function AlertRow({ alert }: { alert: ActionItem }) {
  const levelConfig = {
    HIGH: { bg: "bg-red-50", badge: "bg-red-100 text-red-700", icon: <div className="w-2 h-2 rounded-full bg-red-400" /> },
    MEDIUM: { bg: "bg-amber-50", badge: "bg-amber-100 text-amber-700", icon: <div className="w-2 h-2 rounded-full bg-yellow-400" /> },
    LOW: { bg: "bg-emerald-950/20", badge: "bg-emerald-900/30 text-emerald-300", icon: <div className="w-2 h-2 rounded-full bg-blue-400" /> },
  }[alert.level] || { bg: "bg-gray-800", badge: "bg-gray-800 text-gray-200", icon: <span>⚪</span> };
